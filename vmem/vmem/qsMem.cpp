...
int QSMem::Commit(int nByte, int *commited)
{
  if ((mFlags & modeQueue) && (mAllocatedSize != 0))
  {
...
//-----------------share queue-------------------
ShareQueue::ShareQueue(QSMemParams* params)
{
  mLockHandle = new CRITICAL_SECTION;
  if (mLockHandle) { InitializeCriticalSection((LPCRITICAL_SECTION)mLockHandle); }
  mQueue.Init(params);
}
...
{
  if (mLockHandle) {
    DeleteCriticalSection((LPCRITICAL_SECTION)mLockHandle);
    delete mLockHandle;
  }
  mQueue.Final();
}
...
{
  LOCK_EXEC(mQueue.Peek(index, outBuff, nByte, pRead));
}
int ShareQueue::Discard(int nByte, int *pDiscard)
{
  LOCK_EXEC(mQueue.Discard(nByte, pDiscard));
}
int ShareQueue::Read(char *outBuff, int nByte, int *pRead)
{
  LOCK_EXEC(mQueue.Read(outBuff, nByte, pRead));
}
int ShareQueue::Write(char *data, int nByte, int *pWrite)
{
  LOCK_EXEC(mQueue.Write(data, nByte, pWrite));
}
int ShareQueue::Remain()
{
  LOCK_EXEC(mQueue.Remain());
}
int ShareQueue::Count()
{
  LOCK_EXEC(mQueue.Count());
}
int ShareQueue::Allocate(char** pHandle, int size, int *allocatedSize)
{
  LOCK_EXEC(mQueue.Allocate(pHandle, size, allocatedSize));
}
int ShareQueue::Commit(int nByte, int *commited)
{
  LOCK_EXEC(mQueue.Commit(nByte, commited));
}

void ShareQueue::enterLock()
{
  EnterCriticalSection((LPCRITICAL_SECTION)mLockHandle);
}
void ShareQueue::leaveLock()
{
  LeaveCriticalSection((LPCRITICAL_SECTION)mLockHandle);
}